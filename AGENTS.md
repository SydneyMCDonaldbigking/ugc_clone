...
4. 出图的四条规矩(`validate` 会拦后三条):
   - 每次都从已登记的身份图出发:所有镜头使用三视角场景包,人物镜头再用定妆照,只有原片同一时间窗确实出现商品时才加入商品原图;每次尝试都是一次全新生成
...
5. `product_presence` 必须逐参考帧按原片时间窗写成 `present` 或 `absent`:原片没出现商品的镜头不绑定商品图、提示词也不出现商品。参考帧禁止用代码把商品贴到生成背景上;商品镜头直接以场景母版 + 商品原图生成。密集标签无法通过 QC 时,交给 H3 `fully_preserved` 或慢推保底,不做假贴图
...
3. 之后每一段使用对应角度的场景母版,有人物时加入定妆照,只有原片对应时间窗出现商品时才加入商品原图。场景包和定妆照是仅有的两类允许回用的生成图;登记后文件被改动过,`validate` 会报错
...
- 每条 H3 视频的**总参考图数**是 2–3 张。只有该 H3 时间窗包含商品镜头时,使用 1–2 张生成参考帧才可把商品原图放在最后一张补身份;纯食物/动作段使用场景母版补足第二张参考;
...
```json
{
  ...
  "segments": {
    "3": {
      ...
      "notes": "一只手握瓶,背标朝镜头;本镜头按原片时间窗出现商品"
    }
  },
