...
### 配音母带:抓中间文件,不等超分(2026-09-21 实测省 40 分钟/条)

母带的画面是一次性的,**超分纯属白烧**,但没有逐任务的开关:
`--direct-720p` 是唯一关掉二采和超分的 flag,它和 `--width/--height` 互斥,
而且在 `fl2va_8step` 上会 `VRAM grow failed`(GPU 空着 31 GB 却申请 110 MB 失败,重启 ComfyUI 也没用)。

所以走这条路:

1. 用 `--width 480 --height 832` 提交(这个组合稳定出片)
2. 轮询 `/opt/ComfyUI/output/video/h3director/<job>/*二次采样.mp4`,**音频在这一步就写全了**
3. 文件一出现就 `scp` 走,然后把任务从 `jobs/running/` 移出、重启 worker,掐掉后面的超分

实测:`noodle-vo-s` 生成几分钟就完事,后面 40 多分钟全在给一张静止空桌超分。
判断进度看 ComfyUI 日志:刷 `段N二采分块` 是二采(还没写盘),刷 `Model RRDBNet prepared` 就是在超分了(音频早就好了)。

**母带的预审一定会被打回**(静止画面触发 `疑似卡帧`),或者报 `时长异常`;文件仍在 `rejected/<job>/` 或
ComfyUI 的 output 目录里,照常取用。

**压缩后的实测质量**(44 词 / 15 秒 / 176 wpm):基频离散度 **7.1%**,远低于换人线;
5 句里 4 句逐字正确。剩下的错都集中在**连读的专有名词和复合词**:
`Man Xiao Bao` 念成 `manxiao bao`,`in a clear five-kilo bag` 念成 `in a Clive Kilo race`。
改法是断句、拆连字符、去掉前面的 `in a` 这类虚词,让每个词单独立住。

### 第 7 步的自检清单(2026-09-21 补;前一版结论没写到根子上,所以又犯了一遍)

失败不是运气,是三类固定毛病。**A 类最贵,因为它让错误延迟到花完机时才暴露。**

| 类 | 毛病 | 实例 |
|---|---|---|
| A | **验证"命令执行了",不是"效果达成了"** | 读到 `worker: <pid>` 就当重启成功,两条任务在 inbox 躺了十几分钟;钳位后提交成功就宣布上限打通,直到 QA 报 `时长异常: 15.083s`;crown 渲满 20 分钟才暴露 ffmpeg 不在 PATH |
| B | **把注释当证据** | `studio_node.py` 那句"时长秒仅作各段缺省值"骗了两次:先据此以为控件上限不拦(被 ComfyUI 拒),再据此以为段级时长会覆盖控件(出来 15.083s)。**第一次被打脸后只绕开了报错,没回头验证那句话** |
| C | **互斥参数换一个丢一个** | `--direct-720p` ↔ `--width/--height`、`conda run` ↔ `env/bin/python`,来回各栽一次 |

开工前后按这五条做,不是"更小心":

1. **kill 和 start 永远分成两条命令**,中间验证一次。写在同一条里时 `&` 起的进程会被后面的 `kill -9` 连带杀掉
2. **服务操作后验证终态**:worker 起来了 = **任务从 `jobs/inbox/` 挪进 `jobs/running/` 且 GPU 占用上升**,不是 `ps` 里有个 pid
3. **假设用最便宜的探针先验**:验单段时长上限应该提一条 **2 秒**的试,不是 24 秒的正片
4. **产物属性一律 `ffprobe` 量**,不假设参数被遵守。提交参数 ≠ 生效参数
5. **注释是主张不是证据**;被同一处骗过一次必须实测

### 单段时长上限:实测结论(2026-09-21)

**绕不过去,15 秒是硬的。** 三道闸,前两道能改,第三道改了也没用:

1. `src/workflow.py` / `src/rerun.py` 的 `2 <= duration <= 15` —— 可用 `H3_MAX_SEGMENT_SECONDS` 放开
2. ComfyUI 按 `H3DirectorStudio` 的 `INPUT_TYPES` 校验 `时长秒` 控件,`max: 15.0` —— 硬拒 19.033
3. 把控件钳到 `min(duration, 15.0)` 能过校验,但**节点实际就按控件那个 15.0 生成**,段级 duration 不覆盖它。
   提 23.933 秒出来的是 **15.083 秒**,QA 的 `时长异常` 会抓到

所以口播稿必须**压进 15 秒**:约 40–45 词 / 165–175 wpm。
**69 词塞 15 秒(276 wpm)会念坏**:实测吞掉 15 个词,`Man Xiao Bao pouch` 念成 `man shower bow poach`,
两句揉成一句。压缩时保住挂证据的名词和数字,砍画面上本来就看得见的动作描述。
...
`hands_only` 和 `none` 不生成定妆照。每个分镜都从相机角度匹配的已登记场景母版重新生成,人物镜头再加入已登记定妆照;只有原片对应时间窗出现商品时才加入商品原图,
...
含商品的时间窗才把商品原图放最后一张补身份;纯食物/动作段不绑定商品图,单关键帧时用场景母版补足第二张参考。已有 3 张生成 keyframe 时不再加第 4 张。每张图的时间窗和迁移范围必须分开写。
