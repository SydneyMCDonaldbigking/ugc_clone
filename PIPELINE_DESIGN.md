...
9. Generated packaging text is not trusted. Product-present keyframes regenerate directly from the original product photo; dense labels that cannot pass QC use H3 `fully_preserved` or a static packshot fallback.
...
6. Declare `product_presence` per keyframe from the exact source-video time window. Product-absent shots bind no product image; product-present shots use `reference_lock` with the original product photo.
7. Before keyframes, generate and register one empty three-view scene pack (`eye_level`, `oblique_45`, `overhead_90`) from the written source-setting description. Then generate each fresh 9:16 keyframe from the camera-matched scene view, the registered presenter master when applicable, and the original product photo only when the source shot actually shows the product. Do not paste a packshot into a generated background with code. If dense label text cannot pass ImageGen QC, use H3 `fully_preserved` or the static packshot fallback.
...
- every reference has one declared role (`product_identity` or presenter/scene identity); `product_presence=present` requires exactly one product-identity reference and `product_presence=absent` requires none;
...
- code-based product compositing is not part of the keyframe workflow. When exact package pixels matter beyond ImageGen's reliable range, use H3 `fully_preserved` or the static packshot fallback;
...
- **person continuity comes from one presenter master**: a single approved image of the fictional presenter in the job's scene with no product in it. Every applicable keyframe binds that file and its camera-matched scene-pack view; only source windows that actually show the product also bind the original product photo. The scene pack and presenter master are the only generated images allowed as references; registration pins their hashes and `validate` fails if any file changes afterwards;
...
| Product geometry or label drift | One segment | Regenerate directly from the original product photo; if ImageGen cannot pass QC, render with only the product image as H3 `fully_preserved` | `packshot_clip.sh` static slow push |
